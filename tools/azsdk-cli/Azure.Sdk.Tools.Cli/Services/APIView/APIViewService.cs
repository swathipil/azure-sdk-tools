...
public interface IAPIViewService
{
    ...
    Task<string?> GetReviewMetadataAsync(string revisionId);
}
...
public class APIViewService : IAPIViewService
{
    ...
    public async Task<string?> GetReviewMetadataAsync(string revisionId)
    {
        string endpoint = $"/api/Reviews?apiRevisionId={revisionId}";
        string? result = await _httpService.GetAsync(endpoint);

        if (result == null)
        {
            _logger.LogWarning("No review metadata found for revision {RevisionId}", revisionId);
        }
